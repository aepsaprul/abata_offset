            ...
            <div class="form-group col-md-4">
                ...
                <select name="gramasi_id" id="gramasi_id" class="form-control">
                    ...
                    @foreach ($gramasis as $item)
                        <option value="{{ $item->id }}" {{ $item->id == $jenis_kertas->gramasi_id ? 'selected' : '' }}>{{ $item->ukuran }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-md-4">
                <label for="harga" class="form-label">Harga</label>
                <input type="text" class="form-control @error('harga') is-invalid @enderror" id="harga" placeholder="Harga" name="harga" onkeyup="this.value = this.value.toUpperCase()" value="{{ rupiah($jenis_kertas->harga) }}">
                @error('harga')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
