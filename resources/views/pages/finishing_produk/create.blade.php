...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <h3 class="text-center text-uppercase font-weight-bold">Tambah Finishing Produk</h3>
        </div>
    </div>
    <!-- <div class="row justify-content-center"> -->
    <form class="mt-3" action="{{ route('finishing_produk.store') }}" method="POST">
        @csrf
        <div class="row">
            <div class="col-md-12">
        ...
        <div class="row">
            <div class="form-group col-md-6">
                <label for="finishing_id" class="form-label">Nama Finising</label>
                <select class="form-control" id="finishing_id" name="finishing_id">
                    <option value="">-- Pilih Produk --</option>
                    @foreach ($finishings as $finishing)
                        <option value="{{ $finishing->id }}" {{ old('finishing_id') == $finishing->id ? 'selected' : '' }}>{{ $finishing->nama_finishing }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-md-6">
